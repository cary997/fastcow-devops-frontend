...
import { bulkSetUsersRequest, UserCreateRequest } from "@/api/auth/types"
...
import { Delete, Password } from "@vicons/carbon"
import {
  CircleSmall24Filled,
  Edit24Regular,
  Info24Regular,
...
export const usersRules: FormRules = {
  username: {
    ...
    validator(rule: FormItemRule, value: string) {
      ...
      } else if (!usernamePattern.test(value) && !httpUrlPattern.test(value)) {
...
export const createColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      align: "center",
      render(row) {
        return row.user_type == 1 ? $t("usersPage.tableData.local") : "ldap"
      },
    },
    {
      ...
      render(row) {
        const tags = row.roles.map(key => {
          return h(
            NTag,
            {
              type: rolesList.value.length
                ? rolesList.value.find(r => r.id === key).role_status
                  ? "primary"
                  : "error"
                : "default",
              round: true,
            ...
            {
              default: () =>
                rolesList.value.length
                  ? rolesList.value.find(r => r.id === key).nickname
                  : null,
            },
          )
        })
            ...
            hasAuth("auth_users_details")
              ? h("span", { class: "flex flex-row space-x-1" }, [
                  ...
                  h(
                    ...
                    {
                      default: () => $t("usersPage.button.details"),
                    },
                  ),
                ])
              : null,
                        ...
                        h(
                          ...
                          [
                            hasAuth("auth_users_del")
                              ...
                              : null,
                            hasAuth("auth_users_pwdset")
                              ? h(
                                  NButton,
                                  {
                                    size: "tiny",
                                    text: true,
                                    onClick: () => rePassword(row),
                                  },
                                  {
                                    default: () =>
                                      $t("usersPage.button.resetPassword"),
                                    icon: () =>
                                      h(NIcon, {
                                        size: 15,
                                        component: Password,
                            ...
                            hasAuth("auth_users_otpreset")
                              ? h(
                                  ...
                                  {
                                    default: () =>
                                      $t("usersPage.button.resetTotp"),
                                    icon: () =>
                                      h(NIcon, {
                                        size: 15,
                                        component: Password16Filled,
                                      }),
                                  },
                                )
                              : null,
