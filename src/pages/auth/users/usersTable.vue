    ...
    >
      ...
      <template v-slot:tableHeaderAction>
        <Auth value="auth_users_bulkset">
          ...
          >
            <NButton v-show="bulkButtonShow" type="info" size="small">
              <template #icon>
                <n-icon>
                  <PersonEdit20Filled />
                </n-icon>
        ...
        <Auth value="auth_users_add">
          <NButton type="primary" size="small" @click="handleAddUsers">
            <template #icon>
              <n-icon>
                <AddCircle16Filled />
              </n-icon>
    ...
    >
      ...
      <template #action>
        <div class="flex justify-end space-x-4">
          ...
          <n-button size="small" type="primary" @click="handleSubmitClick">
            {{ $t("action.submit") }}
          </n-button>
        </div>
      </template>
  ...
  createColumns({
    ...
    delUser(rowData) {
      window.$dialog.warning({
        ...
        content: t("usersPage.usersDeleteMsg") + "—— " + rowData.nickname,
    ...
    rePassword(rowData) {
      window.$dialog.warning({
        title: t("title.warning"),
        content: t("usersPage.usersRepwdMsg") + "—— " + rowData.nickname,
        positiveText: t("action.confirm"),
        negativeText: t("action.cancel"),
        onPositiveClick: () => {
          const data = {
    ...
    reTotp(rowData) {
      window.$dialog.warning({
        title: t("title.warning"),
        content: t("usersPage.usersReTotpMsg") + "—— " + rowData.nickname,
        positiveText: t("action.confirm"),
        negativeText: t("action.cancel"),
        onPositiveClick: () => {
          resetTotpApi(rowData.id).then(res => {
            if (res.code) {
              window.$dialog.success({
                title: t("title.success"),
                content: t("usersPage.usersResetTotpSuccessMsg"),
                positiveText: t("action.confirm"),
              })
            } else {
