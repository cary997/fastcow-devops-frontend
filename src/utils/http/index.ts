...
import { formatToken, getToken } from "@utils/storage/userLogin"
          ...
          : new Promise(resolve => {
              ...
              if (userInfo) {
                ...
                if (expired) {
                  if (!HttpRequest.isRefreshing) {
                    ...
                    useUserStore()
                      ...
                      .then(res => {
                        ...
                        config.headers!["Authorization"] = formatToken(token)
                        HttpRequest.requests.forEach(cb => cb(token))
                  ...
                  resolve(HttpRequest.retryOriginalRequest(config))
                } else {
